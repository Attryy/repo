...
def similarities(self,linkedin_id, gid):
    X, Y = self.lookupTable.value[idLink], self.lookupTable.value[idGit]
...
def name_similarity(self, Col1, Col2):
    dataFrame['name_leven'] = self.df.apply(lambda row: ld(row[Col1], row[Col2]), axis = 1)
    dataFrame['name_dmetaphone'] = self.df.apply(lambda row: doublemetaphone(row[Col1], row[Col2]), axis = 1)
    dataFrame['name_jw'] = self.df.apply(lambda row: jaro_winkler(row[Col1], row[Col2]), axis = 1)
    dataFrame['name_fuzz'] = self.df.apply(lambda row: fuzz_sort(row[Col1], row[Col2]), axis = 1)
    return self.df
...
def __init__(self, sc,sqlContext, LinkedinData, GithubData):
	...
	LinkedinData= LinkedinData.drop('linkedin_id').withColumnRenamed('linkedin','linkedin_id')
	LinkedinData = LinkedinData.withColumn('skills', F.concat_ws(',',link.skills))
	...
	data_join = self.git.select('id','git_lang','_skills', self.git.bio.alias('_edu_exp'), 'bio', self.git._pro_pub_desc.alias('exp_desc'), 'summary','_pro_pub_title','_pro_pub_desc').union(self.link.select('id',self.link.skills.alias('git_lang'),'_skills','_edu_exp',self.link.headline.alias('bio'), 'exp_desc', 'summary','_pro_pub_title','_pro_pub_desc'))
    ...
    joined = self.git.select('gid', 'linkedin_id', 'git_login','git_name','git_location','git_company','git_blog','github_url','git_websiteUrl').join(self.link.select('linkedin_id','full_name','location','edu_name','exp_org','linkedin_url',self.link.websites.url.alias('website')), ['linkedin_id'])
    joined = joined.withColumn('gid', F.regexp_replace(F.col('gid'), '([\d]+)', 'g$1'))
    count = feature_extraction.joined.cube("linkedin_id").count()
